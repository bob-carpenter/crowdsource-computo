...
An example application analyzed later involves classifying dental X-rays for caries (a kind of pre-cavity).  There are thousands of X-rays, each of which is rated positive or negative for caries by a handful of dentists.  The dentists showed surprisingly little agreement, and very little consensus (complete agreement among dentists for an item), especially in the positive cases.  The dentists also varied dramatically in the number of cases each rated as having caries.  Another example analyzed later is a data set of pairs of sentences, with the goal being to classify pairs where the first sentence implies the second sentence.  This data set has thousands of items and dozens of raters, each of which rates a small subset of the items.  As with the dentistry data, there is surprisingly low agreement for what sounds like a simple task.  A third example is image classification, where digital images are classified by subject matter---one of the earliest applications of crowdsourcing for classification in machine learning was for classifying images of Venus as to whether there was a volcano present.
