...
\\
B
& \lambda_i + (1 - \lambda_i) \cdot \textrm{logit}^{-1}(\alpha^k_j - \beta_i)
\\
BD
...
The following models are redundant in the sense that they reduce to other models, often by introducing an additive and/or multiplicative non-identifiability.  In all of these cases, the reduction is to a single item effect after removing non-identifiability.
