...
This note considers the rating problem for classification.  Although the title says "crowdsourcing," this model is relevant whenever any group of people's opinions need to be combined, be they experts or novices.

An example application analyzed later involves classifying dental X-rays for caries, a kind of pre-cavity [@espeland1989].  There are thousands of X-rays, each of which is rated positive or negative for caries by a handful of dentists.  The dentists showed surprisingly little agreement, and very little consensus (complete agreement among dentists for an item), especially in the positive cases.  The dentists also varied dramatically in the number of cases each rated as having caries.  Another example analyzed later is a data set of pairs of sentences, with the goal being to classify pairs where the first sentence implies the second sentence, the recognizing textual entailment (RTE) task [@snow2008].  This data set has thousands of items and dozens of raters, each of which rates a small subset of the items.  As with the dentistry data, there is surprisingly low agreement for what sounds like a simple task.  A third example is image classification, where digital images are classified by subject matter---one of the earliest applications of crowdsourcing for classification in machine learning was for classifying images of Venus as to whether there was a volcano present.

There are several tasks for which crowdsourcing is used, and a rating model improves performance on all of them over heuristic baselines.  The most common task is to generate a "gold standard" data set, where a single category (or label) is assigned to each item.  In terms of generating representative data, it is best to sample data according to its probability (i.e., follow the generative model) rather than to choose the "best" (i.e., highest probability) rating for each item.  

If the goal of creating a gold standard data set is to train a machine learning classifier, the second section of this paper shows why it is better for downstream accuracy to train with a probabilistic corpus that retains information about rating uncertainty.  Barring that, it is far better to sample labels according to their posterior probability in the rating model than to choose the "best" label.  In particular, majority voting schemes among raters will be later shown to e sub-optimal compared to sampling, which is in turn dominated by training with the probabilities (a kind of Rao-Blackwellization).  An even better approach is to jointly train a classifier and analyze rating data.

A second task for crowdsourcing is to understand a coding standard, which is the instruction set guiding the rating.  Someone generates instructions for how data is to be rated, and its understandability can be understood directly by looking at posterior uncertainty of items, and indirectly by evaluating raters.

A third task for crowdsourcing or rating is to understand the population of raters.  This might be something like the population prevalence of hate speech on a web site.  For that, a rating model will infer a prevalence, perhaps conditioned on item-level covariates.

A fourth task for crowdsourcing is to understand the population of items.  What makes an item hard to annotate?  What makes an item discriminative?  What is the prevalence of positive items?  Population prevalence is the primary focus of disease diagnostic models in epidemiology---the diagnosis of individuals is often taken as secondary.

A fourth task is to understand the population(s) of raters.  This is important when managing raters for multiple crowdsourcing tasks.  For example, it will be possible to infer the rate of spammers and the rate of high quality raters.  The number of raters required for high quality joint ratings may also be assessed.  Raters with high bias or low accuracy may be coached to become better.

A fifth task, though arguably part of the rating task itself, is active learning.  How may items and raters be selected to gather the most information for a task for the least cost?  This problem is related to traditional survey sampling.

One of the main applications of rating models is in crowdsourcing the work of untrained or lightly trained workers.  These same models are also useful for combining expert opinions---experts will just have higher accuracy than non-experts.  
Rating models show up in multiple fields, including educational testing, from where we will draw much of the inspiration for the contribution here, in epidemiology where they are applied to diagnostic tests [@dawid1979,@albert2004], in sociology where they are applied to exploring cultural norms [@romney1986, @batchelder1988], and in data curation for machine learning where they are applied to human feedback on classification or other problems [@smyth1994,@snow2008,@raykar2010].  As further examples, human tags on images are used to train large-scale image classifiers like ImageNet, which has over a million images classified into a thousand classes [@deng2009].  As another example, crowdsourced ratings of language model output are used as a fine-tuning step that adjusts a foundational large language model like GPT-4, which is trained to complete text, into a chatbot like ChatGPT that is (imperfectly) trained to be helpful, truthful, and harmless [@ouyang2022,@rafailov2024].
...
\begin{equation}
y \sim \textrm{Bernoulli}\!\left(\lambda_i + (1 - \lambda_i) \cdot \textrm{logit}^{-1}(\delta_i \cdot (\alpha^k_j - \beta_i))\right),
\end{equation}
...
\\[4pt]
\textrm{Bernoulli}\!\left(1 - \left( \lambda_i + (1 - \lambda_i) \cdot \textrm{logit}^{-1}\!\left(\delta_i \cdot \left(\alpha^\textrm{sens}_j - \beta_i\right)\right)\right)\right)
& \textrm{if } z_i = 0.
\end{cases}
\end{equation}
