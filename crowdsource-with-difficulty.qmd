...
Let $\beta \in \mathbb{R}^K$ be a vector of (unobserved) regression coefficients.  The probability mass function for the category $z_i$ takes the form of a logistic regression,
\begin{equation}
z_i \sim \textrm{bernoulli}\!\left(\textrm{logit}^{-1}(x_i \cdot \beta)\right).
\end{equation}
An intercept may be included as a column of 1 values in the data matrix $x$.  Other regression models could be substituted for the logistic regression, such as using a probit rather than logit link function, or even substituting in a neural network model.  All that is needed is a way to generate a category $z_i$ given the covariates $x_i$.
